...
> **Branch note (`iframe-approach`)**: this branch explores rendering the live preview inside an `<iframe>` pointed at a dedicated `/preview/<template>` route, with parent ↔ iframe communication via `postMessage`. The `master` branch keeps the simpler in-route render where the template is mounted directly inside the builder route. Both share the same Phase 1 + Phase 2 data layer (`useData()` + `TemplateDataProvider` + per-template store); only the *preview shell* differs.
...
5. **Iframe-based preview** (this branch's architecture):
   - **Builder route at `src/app/builder/magicui/page.tsx`** — owns the zustand store and the editor sidebar. Renders an `<iframe src="/preview/magicui">`. Subscribes to the store and posts a `data` message to the iframe on every change. Also listens for the iframe's `ready` message to push the latest data on initial mount (avoids race between iframe load and first store change).
   - **Preview route at `src/app/preview/magicui/{layout.tsx, page.tsx}`** — loaded inside the iframe. The layout mounts the template's `<ThemeProvider>` + `<TooltipProvider>` and applies `geist.variable` / `geistMono.variable` so the template's `font-sans` utilities resolve. The page is `"use client"`, holds incoming data in `useState<Data | null>(null)`, listens for `message` events, and wraps `<MagicuiPage />` + `<Navbar />` in `<TemplateDataProvider value={data}>`. Posts a `ready` sentinel back to `window.parent` on mount.
   - **Cross-frame protocol** at `src/lib/preview-protocol.ts` — typed discriminated union `{ source: "few-clicks-preview"; type: "ready" | "data"; payload?: Data }` plus an `isPreviewMessage` type guard. The `source` sentinel filters out unrelated `postMessage` traffic (browser extensions, OAuth popups, etc.). All `postMessage` / `addEventListener` calls use `window.location.origin` as the target / origin filter rather than `"*"`.
   - **Why iframes**: real CSS isolation (the template can have its own globals, theme tokens, animations without colliding with the builder UI), WYSIWYG vs export (the iframe content is what the standalone export will render), and clean scaling to multiple templates (each gets its own `/preview/<template>` route with its own layout chrome). Cost: a small `postMessage` protocol and the iframe being its own React tree (no shared hydration tree with the builder).
   - **Initial render**: when no `data` message has arrived yet, `<TemplateDataProvider value={null}>` causes `useData()` to fall back to the static `DATA`. So the iframe is never blank on load — it shows the template's seed data until the first message lands.
6. **Shared template-side assets** — fonts were extracted from `app/layout.tsx` into `lib/fonts.ts` (`geist`, `geistMono` from `next/font/google`). Both the standalone template's `layout.tsx` and the iframe's preview layout import from there. When a template needs assets that the builder also has to apply (fonts, theme tokens, future shared shells), put them under the template's `lib/` so they ship with the export and stay the single source of truth.
